...
def render(context, params):
    ...
    filtered_pages_names = filtered_pages.annotate(full_name=Concat('category', V(':'), 'name', output_field=TextField(db_collation="en_ci"))).values('full_name')
    
    all_articles = Article.objects.values('name', 'category', 'title') \
      .annotate(full_name=Concat('category', V(':'), 'name', output_field=TextField(db_collation="en_ci")))
    q = ExternalLink.objects.filter(link_type='link') \
      .annotate(link_from_complete=Case(When(~Q(link_from__contains=':'), then=Concat(V('_default:'), 'link_from', output_field=TextField(db_collation="en_ci"))), default='link_from')) \
      .filter(link_from_complete__in=Subquery(filtered_pages_names)) \
      .annotate(link_to_complete=Case(When(~Q(link_to__contains=':'), then=Concat(V('_default:'), 'link_to', output_field=TextField(db_collation="en_ci"))), default='link_to'))
