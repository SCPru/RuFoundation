                ...
                {% for category, tags in categories.items %}
                    <div class="w-collapsible collapsible-block">
                        <div class="collapsible-block-folded" style="display: none;">
                            <h2 class="collapsible-block-link"><a href="javascript:;">+ {{ category.name }}{% if not category.is_default and category.name != category.slug %} <pre style="display: inline; font-size: 60%">[{{ category.slug }}]</pre>{% endif %}</a></h2>
                        </div>
