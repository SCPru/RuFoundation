    ...
    try:
        match log_entry.type:
            case ActionType.Vote:
                m = _mark_safe_all(m)
                if m['is_new']:
                    return f'На страницу {m['article']} добавлена оценка {m['new_vote']:.1f}'
                elif m['is_remove']:
                    return f'Со страницы {m['article']} удалена оценка {m['old_vote']:.1f}'
                elif m['is_change']:
                    return f'Оценка на странице {m['article']} изменена с {m['old_vote']:.1f} на {m['new_vote']:.1f}'
                else:
                    return f'Оценка на странице {m['article']} удалена повторно (ее не было)'
