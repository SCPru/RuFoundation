...
const TagEditorComponent: React.FC<Props> = ({ tags, allTags, onChange, canCreateTags }) => {
  ...
  const categoriesAndTags = allTags.categories
    .map(category => {
      return {
        name: category.name,
        slug: category.slug,
  ...
  const filteredCategories = allTags.categories
    .map(category => ({
      ...category,
    ...
    .map(category => ({
      ...category,
      tags:
        category.slug.toLowerCase().indexOf(inputValue.toLowerCase()) !== -1 || `${category.slug.toLowerCase()}:` === inputValue
          ? category.tags
          : category.tags.filter(
              x =>
                !tags.find(y => y.toLowerCase() === `${category.slug}:${x.name}`.toLowerCase()) &&
                x.name.toLowerCase().indexOf(inputValue.toLowerCase()) !== -1,
            ),
    }))
    .filter(x => !!x.tags.length)
  ...
  const onInputKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Enter' && selectedToAdd && currentCategoryIndex !== undefined) {
      ...
      setSelectedToAdd(
        currentCategoryIndex !== undefined
          ? `${filteredCategories[currentCategoryIndex].slug}:${currentTagIndex !== undefined ? filteredCategories[currentCategoryIndex].tags[currentTagIndex].name : ''}`
          : undefined,
      )
    }
  }
      ...
      <TagListContainer>
        {categoriesAndTags.map(category => (
          <React.Fragment key={category.name}>
            <TagCategoryName>{category.name}</TagCategoryName>
            <TagList>
            ...
            {filteredCategories.map(category => (
              <React.Fragment key={category.slug}>
                <TagSuggestionCategory
                  ...
                  className={selectedToAdd === `${category.slug}:` ? 'active' : ''}
                  onClick={e => onSelectCategory(e, category.slug)}
                >
                  {category.name} [{category.slug}]
                </TagSuggestionCategory>
                {category.tags.map(tag => (
                  <TagSuggestionTag
                    data-selector={`${category.slug}:${tag.name}`}
                    key={tag.name}
                    className={selectedToAdd === `${category.slug}:${tag.name}` ? 'active' : ''}
                    onClick={e => onSelectTag(e, `${category.slug}:${tag.name}`)}
                  >
                    {tag.name}
                  </TagSuggestionTag>
                ))}
